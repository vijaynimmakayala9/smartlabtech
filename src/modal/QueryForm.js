...
import { X, Send, CheckCircle2, MessageCircleQuestion, User, Phone, Mail, Building2 } from 'lucide-react';
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-2.5 sm:gap-3.5">
            <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-lg sm:rounded-xl bg-white/15 backdrop-blur-sm flex items-center justify-center border border-white/20">
              <MessageCircleQuestion size={18} className="sm:w-5 sm:h-5" color="#fff" />
            </div>
            <div>
          ...
          <button onClick={onClose} className="w-8 h-8 sm:w-[34px] sm:h-[34px] rounded-lg sm:rounded-xl bg-white/15 border border-white/20 cursor-pointer flex items-center justify-center text-white hover:bg-white/25 transition-all">
          ...
          {done ? (
            ...
            <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <Field label="Your Query *">
                <Input as="textarea" rows={5} placeholder="Describe your query in detail..." value={form.message} onChange={set('message')} error={errors.message} />
              </Field>
